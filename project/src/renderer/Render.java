...
public class Render {
	...
	private Entry<Geometry,Point3D> getClosestPoint(Map<Geometry,List<Point3D>> intersectionPoints) {
		...
		Point3D P0 = scene.getCamera().getCenter();
	...
	private Color calcColor(Geometry geometry, Point3D point, Ray ray){
		...
		private Color calcColor(Geometry geometry, Point3D point,Ray inRay,int level) {
			...
			while (lights.hasNext()){
				...
				Color lightSpecular = calcSpecularComp(geometry.getMaterial().getKs(),new Vector(point, scene.getCamera().getCenter()),geometry.getNormal(point),light.getL(point),geometry.getMaterial().getN(), lightIntensity);
